...
void StringBuilderRemove(StringBuilder *sb)
{
	sb->data = realloc(sb->data, sizeof(char*) * --sb->size);
}
...
StringBuilder *StringBuilderCreateFromString(const char *str, const char *delim)
{
	...
	StringBuilder *sb = StringBuilderCreateFromStrings(split);
	splitfree(split);
	return sb;
}
...
{
	char **str = StringBuilderToStrings(sb, false);
	return StringBuilderCreateFromStrings(str);
}

/* concat sb2 to sb1, sb2 is unharmed */
void StringBuilderConcat(StringBuilder *sb1, StringBuilder *sb2)
{
	char **str = StringBuilderToStrings(sb2, false);
	while (*str) {
		StringBuilderAppend(sb1, *str);
		str++;
	}
}
